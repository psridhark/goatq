...
func (b *Node) setupRaft() (err error) {
	...
	if b.config.DevMode {
		...
		path := filepath.Join(b.config.RaftDir, raftState)
...
func (b *Node) reconcileReaped(known map[int32]struct{}) error {
	state := b.fsm.State()
...
func (b *Node) reconcileMember(m serf.Member) error {
	var err error
	switch m.Status {
...
func (b *Node) handleAliveMember(m serf.Member) error {
	...
	req := structs.RegisterNodeRequest{
		Node: structs.Node{
			...
			GRPCAddress: meta.GrpcAddr,
			Meta: map[string]string{
				"raft_addr":     meta.RaftAddr,
				"db_raft_addr":  meta.DBRaftAddr,
...
func (b *Node) raftApply(t structs.MessageType, msg interface{}) (interface{}, error) {
...
func (b *Node) handleLeftMember(m serf.Member) error {
	return b.handleDeregisterMember("left", m)
}

func (b *Node) handleReapMember(member serf.Member) error {
	return b.handleDeregisterMember("reaped", member)
}

// handleDeregisterMember is used to deregister a mmeber for a given reason.
func (b *Node) handleDeregisterMember(reason string, member serf.Member) error {
	meta, ok := metadata.IsBroker(member)
	if !ok {
		return nil
	}
...
func (b *Node) joinCluster(m serf.Member, parts *metadata.Broker) error {
	if parts.Bootstrap {
		members := b.LANMembers()
		for _, member := range members {
...
func (b *Node) handleFailedMember(m serf.Member) error {
	// TODO: Implement this
	//meta, ok := metadata.IsBroker(m)
	//if !ok {
	//	return nil
...
func (b *Node) removeServer(m serf.Member, meta *metadata.Broker) error {
	configFuture := b.raft.GetConfiguration()
	if err := configFuture.Error(); err != nil {
		log.Error.Printf("leader/%d: get raft configuration error: %s", b.config.ID, err)
		return err
