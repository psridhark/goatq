...
func (b *Node) setupSerf(config *serf.Config, ch chan serf.Event, path string) (*serf.Serf, error) {
	config.Init()
	fmt.Printf("THE serf Cofnig : %v\n", config)
	//config.NodeName = fmt.Sprintf("%s%d", b.config.NodeName, b.config.ID)
	config.NodeName = fmt.Sprintf("%d", b.config.ID)
	config.Tags["role"] = "goatq"
	config.Tags["id"] = fmt.Sprintf("%d", b.config.ID)
	config.MemberlistConfig.AdvertiseAddr = b.config.SerfLANConfig.MemberlistConfig.BindAddr
	config.MemberlistConfig.AdvertisePort = b.config.SerfLANConfig.MemberlistConfig.BindPort
	...
	config.Tags["db_raft_addr"] = b.config.DBRaftAddr
	config.Tags["http_addr"] = b.config.HttpAddr
	config.Tags["grpc_addr"] = b.config.GrpcAddr
	tmpAddr := fmt.Sprintf("%s:%d", b.config.SerfLANConfig.MemberlistConfig.BindAddr, b.config.SerfLANConfig.MemberlistConfig.BindPort)
	fmt.Println("serf_lan_addr : ", tmpAddr)
	...
	if !b.config.DevMode {
		config.SnapshotPath = filepath.Join(b.config.RaftDir, path)
	}
...
func (b *Node) lanEventHandler() {
	for {
		select {
		case e := <-b.eventChLAN:
			switch e.EventType() {
			case serf.EventMemberJoin:
				fmt.Println("GOT A EventMemberJoin event ", serf.EventMemberJoin)
...
func (b *Node) lanNodeJoin(me serf.MemberEvent) {
...
func (b *Node) lanNodeFailed(me serf.MemberEvent) {
	for _, m := range me.Members {
		meta, ok := metadata.IsBroker(m)
		if !ok {
			continue
...
func (b *Node) localMemberEvent(me serf.MemberEvent) {
	if !b.isLeader() {
		return
	}
...
func (b *Node) maybeBootstrap() {
	...
	if index != 0 {
		log.Info.Printf("broker/%d: raft data found, disabling bootstrap mode: index: %d, path: %s", b.config.ID, index, filepath.Join(b.config.RaftDir, raftState))
